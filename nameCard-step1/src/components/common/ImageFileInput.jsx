...
// 화면이 다시 그려지면 함수도 새로 만들어진다. - 비효율적임 
const ImageFileInput = ({name, imageUploader}) => {
  // input type file은 숨기고 버튼을 눌렀을때 file컴포넌트 동작시키기 
  // file 컴포넌트가 loading과 같은 상태가 바뀔때마다 다시 생성된다. ???
  // 상태가 바뀌더라도 원래값을 유지해주는 훅이 있다. -useRef
  const inputRef = useRef() //앞에  use로 시작되는 함수는 모두 Hook임  
  // file명을 선택하고 업로드 중일때 상태 관리 
  ...
  const handleChange = async (event) => {
    // 로딩 애니매이션 효과 발동 -> 상태값을 false -> true 변경해줌
    setLoading(true)

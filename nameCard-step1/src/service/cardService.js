...
class CardLogic { // 클래스 선언 
  constructor(app){ // 생성자 함수 
    //getDatabase(app)로 RealtimeDatabase 인스턴스를 얻어옴 
    //this가 붙은 전변이다. 다른 매서드에서 재사용 가능 
  ...
  syncCards(userId, onUpdate) {
    ...
    // 해당 경로의 데이터가 변경될때마다 자동으로 실행됨(실시간)
    onValue(query, (snapshot) => {
      // 데이터를 받아오기
      const value = snapshot.val();
      // value가 있으면 onUpdate 호출해서 화면 상태를 갱신 - 구독하기 
      value && onUpdate(value);
    });
    //중요 : 리스너 해제(unsubscribe 패턴)
    //clean-up 할때 필요한 문장임 
    return () => off(query);
  }
//https://firebase.google.com/docs/database/web/read-and-write?authuser=0
//목적: 한장의 명함카드 정보를 DB에 저장(덮어쓰기 방식)
...
//만일 같은 id로 다시 저장하면 수정 효과가 발생함. 
  ...
  //특정 카드 1개 삭제 
  //remove 함수의 ref 위치의 데이터 자체를 제거함. 
